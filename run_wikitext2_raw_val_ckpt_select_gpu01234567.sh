...
# WikiText-2 Raw-SetBCE validation-checkpoint selection.
# This mirrors the OPAL validation-checkpoint script, but trains/evaluates
# the raw-prefix router so the best-on-val comparison is fair.

export REPO_DIR="${REPO_DIR:-/workspace/PriorDynamicPruning}"
export CUDA_DEVICE_ORDER="${CUDA_DEVICE_ORDER:-PCI_BUS_ID}"
export CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-0,1,2,3,4,5,6,7}"
export NUM_GPUS="${NUM_GPUS:-8}"
export BASE_PORT="${WIKITEXT_BASE_PORT:-58700}"
...
export WIKITEXT_VALCKPT_DIR="${WIKITEXT_VALCKPT_ROOT}/raw_embedding_${WIKITEXT_LOSS_DIR_TAG}"
...
' "$port"; do
    ...
    local port_log
    port_log="$(mktemp "/tmp/wikitext2_raw_valckpt_${port}_XXXX.log")"
    set +e
    accelerate launch \
...
eval_epoch_checkpoint_single_gpu() {
  ...
  local out="${WIKITEXT_VAL_METRIC_DIR}/raw_epoch${epoch_tag}_validation.json"
  ...
  echo "=== GPU ${gpu_id}: evaluate raw epoch ${epoch_tag} ==="
  ...
  python3 ./eval_wikitext_opal_ppl.py eval \
    ...
    --method router \
    --method_label "Raw-${WIKITEXT_SET_LOSS_TYPE} epoch${epoch} validation" \
    ...
    --run_name "${WIKITEXT_RUN_ID}_raw_epoch${epoch_tag}_validation" \
...
eval_all_epoch_checkpoints_parallel() {
  ...
  echo "=== Parallel raw validation sweep: ${worker_count} single-GPU workers over ${WIKITEXT_EPOCHS} checkpoints ==="
  ...
  for worker_idx in $(seq 0 "$((worker_count - 1))"); do
    ...
    echo "=== Start raw validation worker ${worker_idx} on GPU ${gpu_id}; log ${log_path} ==="
    (
      set -euo pipefail
  ...
  if [ "$status" -ne 0 ]; then
    echo "=== At least one raw validation worker failed. Log tails follow. ===" >&2
    local log_path
  ...
  local epoch
  for epoch in $(seq 1 "$WIKITEXT_EPOCHS"); do
    local out="${WIKITEXT_VAL_METRIC_DIR}/raw_epoch$(printf "%03d" "$epoch")_validation.json"
    if ! json_usable "$out"; then
      echo "Missing or unusable validation metric after raw parallel sweep: ${out}" >&2
      missing=1
    fi
  done
  ...
  echo "=== Parallel raw validation sweep complete ==="
}

if ! label_file_usable "$WIKITEXT_LABEL_FILE" "$WIKITEXT_LABEL_SAMPLES"; then
  echo "Missing or poisoned greedy labels: ${WIKITEXT_LABEL_FILE}" >&2
  echo "Run run_wikitext2_public_lm_sanity_gpu01234567.sh once with a clean label run id first." >&2
  exit 2
fi

echo "=== WikiText-2 Raw-SetBCE validation-checkpoint selection ==="
...
if [ -s "$last_ckpt" ]; then
  echo "=== Reuse raw epoch checkpoints through ${last_ckpt} ==="
else
  if [ "$WIKITEXT_VALCKPT_EVAL_ONLY" = "1" ]; then
    echo "Missing raw epoch checkpoints through ${last_ckpt}; WIKITEXT_VALCKPT_EVAL_ONLY=1, so stop instead of retraining." >&2
    exit 2
  fi
  echo "=== Train Raw router (${WIKITEXT_SET_LOSS_TYPE}) with per-epoch checkpoints ==="
...
echo "=== Evaluate every raw epoch checkpoint on validation ==="
eval_all_epoch_checkpoints_parallel

echo "=== Select best raw validation checkpoint ==="
...
rows = []
for path in sorted(glob.glob(os.path.join(metric_dir, "raw_epoch*_validation.json"))):
    payload = json.load(open(path))
...
if not rows:
    raise SystemExit("No raw validation metrics found.")
eligible = [row for row in rows if int(row["unique_masks"]) >= min_unique]
if not eligible:
    raise SystemExit(f"No raw validation metrics satisfy min_unique_masks={min_unique}.")
...
print(f"Raw validation checkpoint table: min_unique_masks={min_unique}, eligible_epochs={len(eligible)}/{len(rows)}")
...
if min_unique > 0:
    print(f"\nTop eligible raw checkpoints with unique_masks >= {min_unique}:")
    print("| epoch | val_NLL | val_PPL | unique_masks | exact-K | dominant_mask_count | dominant_mask |")
    print("|---:|---:|---:|---:|---:|---:|---|")
...
test_json="${WIKITEXT_VAL_METRIC_DIR}/raw_best_val${selection_suffix}_epoch$(printf "%03d" "$best_epoch")_test.json"
if json_usable "$test_json"; then
  echo "=== Reuse raw best-checkpoint test metric: ${test_json} ==="
else
  echo "=== Evaluate best raw validation checkpoint on test: epoch ${best_epoch} ==="
  run_accelerate ./eval_wikitext_opal_ppl.py eval \
    ...
    --eval_windows "$WIKITEXT_EVAL_WINDOWS" \
    --method router \
    --method_label "Raw-${WIKITEXT_SET_LOSS_TYPE} best-on-val${selection_suffix} epoch${best_epoch}" \
    ...
    --protected_head "$WIKITEXT_PROTECTED_HEAD" \
    --protected_tail "$WIKITEXT_PROTECTED_TAIL" \
    --run_name "${WIKITEXT_RUN_ID}_raw_best_val${selection_suffix}_epoch$(printf "%03d" "$best_epoch")_test" \
    --batch_size "$WIKITEXT_EVAL_BATCH_SIZE" \
...
echo "=== Compact raw validation-checkpoint result ==="
...
echo "=== Done: WikiText-2 Raw-SetBCE validation-checkpoint selection ${WIKITEXT_RUN_ID} ==="
