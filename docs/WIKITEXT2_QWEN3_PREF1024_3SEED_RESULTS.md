...
> On clean Qwen3-8B WikiText-2 with 25% layer skipping (K=9), increasing the router-visible prefix from 256 to 1024 tokens makes validation-selected OPAL outperform the corresponding Raw router across three seeds. The best mean PPL is OPAL BCE best-on-val (`17.5974`), ahead of Raw BCE best-on-val (`17.7729`). However, the OPAL BCE selected checkpoints are low-diversity, so this should be framed as a validation-selected prefix1024 OPAL rescue rather than evidence that high mask diversity is the source of the gain.
