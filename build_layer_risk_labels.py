...
def main():
    parser = argparse.ArgumentParser(description="Build per-input one-layer-drop skip risk labels.")
    ...
    iterator = tqdm(dataloader, desc="risk-labels") if accelerator.is_main_process else dataloader
    with shard_path.open("w", encoding="utf-8") as f, torch.no_grad():
        for batch in iterator:
            ...
            for layer_idx in range(num_layers):
                mask = torch.ones((input_ids.size(0), num_layers), dtype=torch.float32, device=device)
                mask[:, layer_idx] = 0.0
                drop_logits = forward_with_mask(model, input_ids, attention_mask, mask)
                quality_rows = quality_rows_from_logits(full_logits, drop_logits, labels)
                for row_pos, quality in enumerate(quality_rows):
                    risk = (
                        float(quality["KL_full_to_skip"])
                        if args.objective == "KL"
                        else float(quality["Delta_NLL"])
                    )
                    if risk_rows[row_pos]["full_nll"] is None:
                        risk_rows[row_pos]["full_nll"] = float(quality["NLL_full"])
                    risk_rows[row_pos]["risk_labels"].append(risk)
                    risk_rows[row_pos]["drop_nll"].append(float(quality["NLL_skip"]))
                    risk_rows[row_pos]["drop_kl"].append(float(quality["KL_full_to_skip"]))
                    risk_rows[row_pos]["drop_delta_nll"].append(float(quality["Delta_NLL"]))
    ...
    if accelerator.is_main_process:
        ...
        shard_rows.sort(key=lambda row: int(row["sample_id"]))
        ...
        print(f"Wrote {len(shard_rows)} risk-label rows to {output_path}")
